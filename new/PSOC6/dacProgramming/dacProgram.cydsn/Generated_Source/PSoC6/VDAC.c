...
*
* Description:
*  This file provides the source code to the API for the VDAC
*  component.
*
* Note:
*  None.
...
uint8_t VDAC_initVar = 0u;

const cy_stc_ctdac_config_t VDAC_ctdacConfig =
{
    .refSource          = VDAC_CTDAC_REFSOURCE,
    .formatMode         = VDAC_CTDAC_SIGNMODE,
    .updateMode         = VDAC_CTDAC_UPDATEMODE,
    .deglitchMode       = VDAC_CTDAC_DEGLITCHMODE,
    .outputMode         = CY_CTDAC_OUTPUT_VALUE,
    .outputBuffer       = VDAC_CTDAC_OUTPUTBUFFER,
    .deepSleep          = VDAC_CTDAC_DEEPSLEEP,
    .deglitchCycles     = VDAC_DEGLITCH_TARGET_CYCLES,
    .value              = VDAC_PARAM_InitialCode,
    .nextValue          = VDAC_PARAM_InitialCode,
    ...
    {
        .deepSleep         = VDAC_CTB_VOUTBUF_DEEPSLEEP,
        .oaPower           = VDAC_CTB_VOUTBUF_POWER,
        .oaMode            = VDAC_CTB_VOUTBUF_DRIVE,
        .oaPump            = VDAC_CTB_VOUTBUF_PUMP,
...
#endif /* VDAC_PARAM_OutputBufferUsed */

#if VDAC_PARAM_ReferenceBufferUsed
    const cy_stc_ctb_opamp_config_t VDAC_refBufferConfig =
    {
        .deepSleep         = VDAC_CTB_REFBUF_DEEPSLEEP,
        .oaPower           = VDAC_CTB_REFBUF_POWER,
        .oaMode            = VDAC_CTB_REFBUF_DRIVE,
        .oaPump            = VDAC_CTB_REFBUF_PUMP,
        .oaCompEdge        = CY_CTB_COMP_EDGE_DISABLE,
        .oaCompLevel       = CY_CTB_COMP_DSI_TRIGGER_OUT_PULSE,
        .oaCompBypass      = CY_CTB_COMP_BYPASS_SYNC,
        .oaCompHyst        = CY_CTB_COMP_HYST_DISABLE,
...
#endif /* VDAC_PARAM_ReferenceBufferUsed */

/** The instance-specific context structure.
* It is used in the VDAC_DeepSleepCallbackParams for managing the deglitch mode.
* The user should not modify anything in this structure.
*/
static cy_stc_ctdac_context_t VDAC_ctdacContext;

static cy_stc_syspm_callback_params_t VDAC_DeepSleepCallbackParams = {
    .base = VDAC_CTDAC_HW,
    .context = (void *) &VDAC_ctdacContext,
};

cy_stc_syspm_callback_t VDAC_DeepSleepCallbackStruct = {
    .callback = &Cy_CTDAC_DeepSleepCallback,
    .type = CY_SYSPM_DEEPSLEEP,
    .callbackParams = &VDAC_DeepSleepCallbackParams,
};
...
****************************************************************************//**
*
*  Initializes the VDAC with default customizer
*  values when called the first time (after reset) and enables the VDAC.
*  For subsequent calls the configuration is left unchanged and the VDAC is
*  just enabled.
*
*  VDAC_initVar: this global variable is used to indicate the initial
*  configuration of this component. The variable is initialized to zero and set
*  to 1 the first time VDAC_Start() is called. This allows
*  enabling/disabling a component without re-initialization in all subsequent
*  calls to the VDAC_Start() routine.
*
*******************************************************************************/
void VDAC_Start(void)
{
    if(0u == VDAC_initVar)
    {
       ...
       VDAC_initVar = 1u;
        ...
        && VDAC_COMP_AREF_CURRENT_LOW_USED)
...
/*******************************************************************************
* Function Name: VDAC_SetDeepSleepMode
****************************************************************************//**
*
...
{
#if (VDAC_PARAM_DeepSleep != VDAC_COMP_DEEPSLEEP_OFF)

    switch(deepSleepMode)
    {
    case CY_CTDAC_DEEPSLEEP_DISABLE:
        Cy_CTDAC_SetDeepSleepMode(VDAC_CTDAC_HW, CY_CTDAC_DEEPSLEEP_DISABLE);

        /* Disable the CTB for Deep Sleep only if both opamps are used */
    #if (VDAC_PARAM_ReferenceBufferUsed && VDAC_PARAM_OutputBufferUsed)
        Cy_CTB_SetDeepSleepMode(VDAC_CTB_HW, CY_CTB_DEEPSLEEP_DISABLE);
    #endif
        break;
    case CY_CTDAC_DEEPSLEEP_ENABLE:
    default:
        Cy_CTDAC_SetDeepSleepMode(VDAC_CTDAC_HW, CY_CTDAC_DEEPSLEEP_ENABLE);

        /* Enable the CTB for Deep Sleep if any opamps are used */
    #if (VDAC_PARAM_ReferenceBufferUsed || VDAC_PARAM_OutputBufferUsed)
        Cy_CTB_SetDeepSleepMode(VDAC_CTB_HW, CY_CTB_DEEPSLEEP_ENABLE);
    #endif

        break;
    }

#endif /* (VDAC_PARAM_DeepSleep != VDAC_COMP_DEEPSLEEP_OFF) */
}

/*******************************************************************************
* Function Name: VDAC_SetSampleAndHold
****************************************************************************//**
*
* Performs one of three sample and hold modes.
*
*   - To sample the DAC output, call with \ref VDAC_SH_SAMPLE.
*     It takes 10 us for the voltage to be sampled on the S/H capacitor.
*   - To hold the DAC output before entering Deep Sleep, call with \ref VDAC_SH_HOLD.
...
*
* See VDAC_EasySampleAndHold() for a complete sequence of
...
*
* \param mode
* - \ref VDAC_SH_SAMPLE : Sample the DAC output
* - \ref VDAC_SH_HOLD : Hold the DAC output
* - \ref VDAC_SH_DISABLE : Disconnect the S/H capacitor
*
*******************************************************************************/
void VDAC_SetSampleAndHold(VDAC_en_sample_hold_t mode)
{
#if (VDAC_PARAM_OutputBufferUsed && (VDAC_PARAM_SampleAndHold > 0uL))
    switch(mode)
    {
    case VDAC_SH_SAMPLE:
        Cy_CTB_DACSampleAndHold(VDAC_CTB_HW, CY_CTB_SH_PREPARE_SAMPLE);

        /* Enable DAC output */
        Cy_CTDAC_SetOutputMode(VDAC_CTDAC_HW, CY_CTDAC_OUTPUT_VALUE);

        Cy_CTB_DACSampleAndHold(VDAC_CTB_HW, CY_CTB_SH_SAMPLE);
        break;
    case VDAC_SH_HOLD:
        Cy_CTB_DACSampleAndHold(VDAC_CTB_HW, CY_CTB_SH_PREPARE_HOLD);

        /* Disable DAC output */
        Cy_CTDAC_SetOutputMode(VDAC_CTDAC_HW, CY_CTDAC_OUTPUT_HIGHZ);

        Cy_CTB_DACSampleAndHold(VDAC_CTB_HW, CY_CTB_SH_HOLD);
        break;
    case VDAC_SH_DISABLE:
    default:
        Cy_CTB_DACSampleAndHold(VDAC_CTB_HW, CY_CTB_SH_DISABLE);

        /* Enable DAC output */
        Cy_CTDAC_SetOutputMode(VDAC_CTDAC_HW, CY_CTDAC_OUTPUT_VALUE);
        break;
    }
#endif /* (VDAC_PARAM_OutputBufferUsed && (VDAC_PARAM_SampleAndHold > 0uL)) */
}

/*******************************************************************************
* Function Name: VDAC_EasySampleAndHold
****************************************************************************//**
*
...
* -# Perform sample by calling VDAC_SetSampleAndHold() with VDAC_SH_SAMPLE
* -# Wait 10 us to allow for hold cap to charge
* -# Perform hold by calling VDAC_SetSampleAndHold() with VDAC_SH_HOLD
*
* This function is useful only when the output is buffered
...
{
#if (VDAC_PARAM_OutputBufferUsed && (VDAC_PARAM_SampleAndHold > 0uL))
    if (CTDAC_CTDAC_CTRL_ENABLED_Msk != (VDAC_CTDAC_HW->CTDAC_CTRL & CTDAC_CTDAC_CTRL_ENABLED_Msk))
    {
        Cy_CTDAC_Enable(VDAC_CTDAC_HW);
        Cy_SysLib_DelayUs(2);
    }

    VDAC_SetSampleAndHold(VDAC_SH_SAMPLE);

    Cy_SysLib_DelayUs(10);

    VDAC_SetSampleAndHold(VDAC_SH_HOLD);
#endif /* (VDAC_PARAM_OutputBufferUsed && (VDAC_PARAM_SampleAndHold > 0uL)) */
}
